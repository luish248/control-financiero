(function(){
  ...
  window.saveMovement=async function(){
    ...
    const description=window.selectedType==='gasto'?document.getElementById('fDescriptionSelect').value:document.getElementById('fDescription').value.trim();
    if(category==='__add__'){window.selectedType==='ingreso'?addIncomeCategory():addExpenseCategory();return}
    if(window.selectedType==='gasto'&&description==='__add__') return;
    if(!date||!date.value){alert('Selecciona una fecha.');return}
    if(!amount||amount<=0){alert('Ingresa un monto mayor que 0.');return}
    loading=true;
    try{
      await api('/api/movements',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:window.selectedType,category,amount,detail:description,movement_date:date.value})});
      await load();
      document.getElementById('fAmount').value='';
      document.getElementById('fDescription').value='';
      if(window.selectedType==='gasto'&&typeof renderDescriptionField==='function') renderDescriptionField();
